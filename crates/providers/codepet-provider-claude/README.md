...
The binary implements the generated `Provider` trait and delegates stdio transport to `codepet-provider-sdk::serve_stdio`. The common SDK owns JSON serialization, Provider Frame V1 framing, automatic raw/zstd selection, the encoded-frame limit, bounded normal/control dispatch, overload responses, typed event publication, serialized stdout, terminal cleanup, and shutdown drain; this crate owns only Claude CLI process state and mapping. Provider implementations do not read or write the physical transport themselves.
