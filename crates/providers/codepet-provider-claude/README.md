...
Only the Provider Protocol `workspace-write` access label is advertised and accepted, as the compatibility entry for inherited Claude defaults. It does not force Claude's `acceptEdits` mode. Strong `read-only` and `full-access` modes are not advertised because inheriting arbitrary local configuration cannot guarantee those semantics.

One background reaper owns each `Child`; controls hold only its PID/process group and exit notification. A result becomes terminal only after the process really exits. Instance stop/destroy/shutdown, fatal Provider stdio failures, normal Provider stdin EOF, and interrupt all use bounded process-group termination. Fatal cleanup never depends on Provider stdout remaining writable. Claude physical output lines are limited to 4 MiB, while outward text is split into at most 64 KiB chunks before entering the generated Provider codec's 1 MiB frame limit.
