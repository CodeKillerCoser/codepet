...
impl SessionTraceLinks {
    fn begin_request(&mut self, request: &gateway::ProtocolRequest, context: Option<&gateway::TraceContext>) {
        let (gateway::ProtocolRequest::TurnSend { params, .. }, Some(context)) = (request, context) else {
            return;
        };
...
fn event_turn(event: &gateway::ProtocolEvent) -> Option<&gateway::RoutedResourceId> {
    match event {
        ...
        gateway::ProtocolEvent::ConversationItemUpserted { params, .. } => Some(&params.payload.item.turn),
...
fn event_conversation(event: &gateway::ProtocolEvent) -> Option<&gateway::RoutedResourceId> {
    match event {
        ...
        gateway::ProtocolEvent::ConversationItemUpserted { params, .. } => Some(&params.payload.item.conversation),
...
async fn run_gateway_socket(
    ...
    let (request_tx, request_rx) =
        mpsc::channel::<GatewayRequestDelivery>(REQUEST_QUEUE_CAPACITY);
    ...
    loop {
        ...
        let request = match observed.message {
            gateway::ProviderWireMessage::Request(
        ...
        } = &request
        {
            ...
            event_task = Some(tokio::spawn(async move {
                loop {
                    ...
                    let text = match gateway::encode_event_with_trace(
                        &event,
                        trace_context.as_ref(),
                    ) {
