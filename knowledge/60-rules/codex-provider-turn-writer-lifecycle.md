...
- 成功的 `turn/start`、`turn/steer` 与 `turn/interrupt` response 只建立或保持 active turn，不能把 response 内看似 terminal 的状态当作清理权威：response 处理仍持有 operation lock，此时清理会抢在已排队 notification 前 shutdown writer。turn started/权威 in-progress snapshot 记录 active turn；同 turn 的 item upsert、output delta 和 approval request 也是正在执行的权威证据，必须能修复缺失或滞后的 `turn/started`。只有 `turn/completed` terminal notification 可以在 operation lock 内清除 active turn。租约有效时发布终态后保持 Ready；租约缺失或过期时先切 Closing，但必须先可靠映射并发布 terminal `TurnUpserted`，再 shutdown/kill/wait、按 conversation + generation 删除同一槽并唤醒等待者。
...
- 覆盖 waiting approval、waiting user input、停止续租后的空闲过期、active turn 跨过期、terminal notification、两 conversation 隔离与并发首次 acquire/send。active turn 用可重复短租约场景覆盖：`turn/start` 初始 items 为空且 response 状态看似 terminal，首个 user item 与模型输出分别跨过 reaper 周期，writer 在 terminal event 发布前仍存活。另用过期租约覆盖 `turn/interrupt` response 先返回、`turn/completed` 已排队的顺序，断言 terminal 仍在 slot 移除和进程退出前发布。
