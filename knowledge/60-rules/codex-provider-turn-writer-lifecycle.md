...
- 覆盖 waiting approval、waiting user input、停止续租后的空闲过期、active turn 跨过期、terminal notification、两 conversation 隔离与并发首次 acquire/send。active turn 用可重复短租约场景覆盖：`turn/start` 初始 items 为空、随后收到 user item completed、模型输出跨过至少一个 reaper 周期，writer 在 terminal event 发布前仍存活。
