...
- initialize、subscribe 完成后仍保持 Starting；模型、项目与账号信息后台并行探测全部结束且 generation 当前，才一次发布 Ready，参见 [后台信息规约](provider-background-details.md)；stop 先线性化时，旧 start 返回错误而不能晚到 Ready。
