...
- `runtime`：状态、实际 runtime version/executable path、可选 authentication。
...
Authentication 只允许 `unknown/signed-in/signed-out/expired/error/unsupported` 与可选展示文本，不传 email、account ID、plan 或凭据。用量不随 runtime 摘要传输，统一使用强类型 `codepet.usage.query`，能力由 `usageDatasets` 广告，详见 [用量与存储](../10-architecture/provider-usage-query-and-storage.md)。
...
- 保留 opaque runtime usage：会产生与新用量查询并行的数据口径，现已移除；额度和成本不混入 token 指标。

## 取舍理由

Remote 只依赖稳定 Provider ID、展示身份、当前 runtime 状态和能力 revision。将内部 route 留在 Host，可以独立演进 Provider 实现；将完整能力移出握手，可以避免模型/权限选项放大首包。用量按需查询，避免 runtime 快照承担历史统计。
