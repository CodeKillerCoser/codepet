...
JSON Schema Draft 2020-12 加 method/event manifest 能同时表达跨语言 DTO、版本、方向、幂等性、capability、transport 和 discriminator，且现有生成逻辑已有成熟基础。独立 Rust SDK 让 Provider Host、Gateway 与桌面应用通过普通 crate dependency 使用生成接口，而不是复制代码。compat v0 module 把迁移风险集中在可删除边界：现有 serde fixtures 和 dispatcher 保持不变，新 v1 可以先完成模型与生成验证，再在后续阶段单独接 runtime。
