...
Finder、Dock 或登录项启动的进程通常没有交互式 shell 的完整 `PATH`。Code Pet 的 Runtime 层负责发现、验证和持久化 Agent executable。Codex resolver 只驱动 remote App Server Provider，Claude resolver 只驱动 Claude CLI Provider；Codex Desktop companion 直接连接私有 IPC，不依赖 executable resolver。
...
- Codex/Claude executable 变化时只刷新各自的 remote Provider。
...
- 本阶段不实现 OpenCode Provider，也不实现远程网络 transport。
...
应用启动时，`configured_provider_runtime` 在 Catalog 注册前分别用 resolver 结果覆盖 Codex 和 Claude instance setting。全量刷新、设置或清除 executable 时，`ProviderHostState::refresh_runtime_in_background` 通过每个 Provider 独立的 generation/lock 更新对应 setting，并只重启 `dev.codepet.codex` 或 `dev.codepet.claude`。这个动作不创建 Tauri 内 App Server/Claude adapter，也不操作 companion、Hook 或 Pet。Desktop companion 在另一份 state/registry/event bus 中，由 socket 自行连接和重连，其 generation、owner、revision 和 projection 不变化。
...
- refresh 串 Provider 或误重启 companion：Claude/Codex 使用独立 generation/lock，只更新/restart 目标 Provider；state 边界静态审查确认不操作 companion。
