...
`conversation.get` 先发送 `thread/read(includeTurns: false)` 取得 metadata，再从请求 cursor 开始用 `thread/turns/list(itemsView: full, sortDirection: desc)` 拉取最多 limit 个 turns；limit 默认 40、最大 100，单次上游请求最多 10。Provider 将这批 turns 恢复为时间正序后返回 `pageInfo.nextCursor`，不再读取 cursor 之后的其余历史；返回数量超过请求上限、重复 cursor 或单次请求超过 10,000 个上游页都会 fail closed。新 thread 在首条用户消息前可能已有 metadata，但 App Server 会以 `-32600` 和精确的 `not materialized yet; thread/turns/list is unavailable before first user message` 拒绝第一页；Provider 只把当前 thread id 的这一特定响应解释为空历史，其他 code/message、其他 thread id 或后续页错误仍 fail closed。`conversation.create` 的可读性检查同样把该响应视为 metadata 已就绪，不等待首条消息，并暂存 start response 的权威 metadata；若 observer 在 materialize 前以精确的 `-32600 thread not loaded: <id>` 拒绝 metadata/turns，`conversation.get` 只允许对这个由当前 instance 创建且仍在暂存表中的 id 返回该 snapshot 与空历史。完整 turns 可读后删除暂存，instance stop/fail/restart 也会清空；任意未暂存 id 的 `thread not loaded` 仍 fail closed。该路径不调用 `ensure_thread_loaded`/`thread/resume`，不扫描 transcript，也不写 App Server loaded-thread 状态或创建 writer 执行上下文，因此另一个 runtime 已持有 thread writer 时仍可读取历史。Mapper 保留稳定 item/content ID；user/agent message、reasoning summary、command/file、MCP/dynamic tool 和未知安全占位继续使用相同投影，reasoning 只公开 `summary`，绝不公开 raw `content` 或 `item/reasoning/textDelta`。
...
- `conversation.get` 在另一个 runtime 持 writer 时仍只发送 metadata `thread/read` 与倒序 `thread/turns/list(itemsView=full)` pages，并把当前响应范围恢复为时间正序；重复读取不改变 loaded/configuration 状态、不创建新 session，并继续覆盖 cursor/limit、稳定 item/content ID、active turn、safe unknown、reasoning summary-only 与真实 approval ledger 合并。
