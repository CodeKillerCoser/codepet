...
Prior local probing verified that Codex app-server can send messages into an existing Codex app thread and show them in the app UI. Window activation/open-thread RPC was not verified in the same path, so activation remains separate.

## Validation

- Unit tests: `src-tauri/tests/activity_actions_tests.rs`.
- Frontend capability tests: `frontend/lib/activity.test.ts`.
- Manual validation is required when changing the JSON-RPC method sequence.
