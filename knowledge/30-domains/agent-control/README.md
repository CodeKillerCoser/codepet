...
- `crates/providers/codepet-provider-claude/`：官方 Claude CLI stream-json adapter、诚实降级能力和独立 remote Provider binary。
...
Codex executable 检测和配置只驱动 remote App Server Provider；Claude executable 只驱动 Claude remote Provider。Desktop companion 的可用性只由 Codex Desktop socket 和握手决定；任一路 unavailable 不改变其他 lifecycle 或数据。
