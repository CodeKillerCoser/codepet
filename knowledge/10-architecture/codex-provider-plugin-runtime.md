...
执行槽不归 Remote 详情页、event subscription 或 WSS connection 所有。手机退出或断网不会触发释放；`running`、`waiting-approval`、`waiting-user-input` 都继续保留执行进程。所有 writer route 在取得 conversation operation lock 后统一复核 map 仍指向同一 Ready generation；若请求此前预取的 handle 已变为 Closing/Closed，就丢弃旧 handle、释放旧锁并等待或取得新 generation，绝不向旧 session 写。`turn/completed` 携带 `completed/failed/interrupted` 时，Provider 在 operation lock 内把 Ready 槽切成 Closing，再发布权威 event、关闭旧子进程、按 conversation + generation 移除同一槽并唤醒等待者。steer/interrupt 后的权威 terminal snapshot 走同一关闭路径。App Server crash、initialize/resume 失败、明确 RPC reject 与 `SentOutcomeUnknown` 也都会关闭并移除对应槽，不自动重放 `turn/start`。
...
- Provider/Gateway v1 的 `conversation.get` 仍返回无分页的完整 items 数组，因此真正任意大历史需要后续公共协议分页；本轮只修复能由 App Server turn pages 分解且最终投影不超过 16 MiB 的案例。
