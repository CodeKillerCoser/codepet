...
- 使用统一 IDL 作为协议事实来源。当前生成 Rust SDK、TypeScript Gateway/compatibility SDK 与 Dart Core/Agent/Gateway SDK；未来 Python 必须通过显式 target adapter 接入，在实现前选择即 fail closed。
