...
- `sdk/rust/`：四个可独立编译 SDK；Provider 与 gateway 只依赖 core，pet 只依赖 core。
...
- `src-tauri/src/runtime_gateway/{gateway,event_bus,tauri_bridge}.rs`：未迁移业务，只用现有测试验证兼容和 channel isolation。
