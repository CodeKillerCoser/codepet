...
状态：**M0–M2 已在隔离工作树实现并验证：Windows Host ↔ Android 原生 RTC 的握手、大消息、心跳、事件与撤销通过。尚未合并；M3 公网信令与 coturn 已部署、Windows 强制 TURN/UDP 互通通过，M4 手机跨网和网络迁移验收待完成。默认仍为 LAN/WSS。** 用户随后指定创建并入库固定 Debug key；Debug APK 已构建且证书核验一致，Release 仍要求外部正式签名。
...
| M3 公网连接 | HTTPS 信令服务与两端 adapter、准入、TURN 配置、连接模式组合；依赖 M2 | 已配对设备离开 LAN 后能鉴权建链；强制 TURN 测试；LAN 默认仍可用 | 待验收 |
| M4 回归交付 | 双通道业务契约矩阵、大陆网络/真机验证、运维说明；依赖 M3 | 以下验收矩阵全部有结果，剩余限制明确 | 待验收 |
...
已在现有美国加州 VPS 部署 HTTPS 8443 信令与 coturn；公网 IP 证书和自动续期已验证，原有 xray 443 保留。详见 [公网部署](../40-runbooks/webrtc-public-signaling.md)。实测 UDP、TURN/TCP、TURN/TLS，记录大陆 Wi-Fi、移动数据、运营商、直连/relay、建链耗时和失败原因；失败不能仅凭 IP 归因为带宽或地域。
...
已实现独立信令服务、设备签名绑定、可信 LAN bootstrap、公网 RTC 路由与短期 TURN 凭据。Windows 强制 TURN/UDP 的公网信令、选中中继、握手、大消息与撤销通过。Remote 最终 107 项回归通过；手机锁定导致真机安装等待，本次手机公网测试未执行。Rust 库目前不支持 TURN/TCP/TLS，已记录明确限制。具体部署、依赖、修复和测试记录以公网部署 runbook 为准。
