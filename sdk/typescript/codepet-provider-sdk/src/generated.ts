// @generated by tools/protocol-codegen/generate.mjs from protocol/provider/v1/manifest.json and protocol/provider/v1/schema.json.
// DO NOT EDIT MANUALLY.

import type { Approval, ApprovalDecision, Conversation, ConversationContentKind, ConversationCreateCapabilities, ConversationItem, ConversationReadState, ConversationStatus, Project, ProjectChangeType, ProjectRoot, ProviderAuthentication, ProviderUsage, TurnInput, TurnSelection, TurnSendCapabilities, TurnTask } from "../../codepet-agent-sdk/src/generated";
export type * from "../../codepet-agent-sdk/src/generated";
import type { ClientConnectionsSnapshot, ClientId, Cursor, DeviceId, JsonObject, NativeResourceId, PageInfo, ProtocolError, ProtocolVersion, ProviderInstanceId, ProviderPluginId, RequestId, RpcError, TimestampMs, VersionRange } from "../../codepet-core-sdk/src/generated";
export type { ClientConnectionsSnapshot, ClientId, Cursor, DeviceId, JsonObject, NativeResourceId, PageInfo, ProtocolError, ProtocolVersion, ProviderInstanceId, ProviderPluginId, RequestId, RpcError, TimestampMs, VersionRange } from "../../codepet-core-sdk/src/generated";

export const PROTOCOL_VERSION = 1 as const;
export const PROTOCOL_METHODS = ["conversation.active.list", "conversation.unread.list", "conversation.markRead", "provider.ping", "provider.initialize", "provider.describe", "runtime.getInstalled", "runtime.select", "instance.create", "instance.start", "instance.stop", "instance.destroy", "instance.capabilities", "conversation.list", "project.list", "project.get", "project.create", "project.update", "project.delete", "conversation.search", "conversation.get", "conversation.acquireInteraction", "conversation.create", "turn.start", "turn.steer", "turn.interrupt", "approval.resolve", "provider.shutdown", "event.subscribe", "event.unsubscribe"] as const;
export const PROTOCOL_EVENTS = ["event.conversationActiveChanged", "event.conversationUnreadChanged", "event.conversationDeleted", "event.projectChanged", "event.instanceStatusChanged", "event.conversationUpserted", "event.conversationItemUpserted", "event.turnUpserted", "event.turnOutputDelta", "event.approvalRequested", "event.approvalResolved", "event.notification", "runtime.inventoryChanged"] as const;

export type JsonValue = null | boolean | number | string | JsonValue[] | { [key: string]: JsonValue };

export interface ApprovalRequestedEvent {
  approval: Approval;
}

export interface ApprovalResolveRequest {
  approval: ProviderResourceId;
  decision: ApprovalDecision;
}

export interface ApprovalResolveResponse {
  approval: Approval;
}

export interface ApprovalResolvedEvent {
  approval: Approval;
}

export interface ConversationAcquireInteractionRequest {
  conversation: ProviderResourceId;
}

export interface ConversationAcquireInteractionResponse {
  selection: TurnSelection;
  leaseExpiresAt?: TimestampMs;
}

export interface ConversationActiveChangedEvent {
  conversation: ProviderResourceId;
  status: ConversationStatus;
  activityVersion: string;
  active: boolean;
  revision: ConversationEnumerationRevision;
}

export interface ConversationActiveEntry {
  conversation: ProviderResourceId;
  status: ConversationStatus;
  activityVersion: string;
}

export interface ConversationActiveListRequest {
  route: ProviderInstanceRoute;
  cursor?: Cursor;
  limit?: number;
}

export interface ConversationActiveListResponse {
  conversations: Array<ConversationActiveEntry>;
  pageInfo: PageInfo;
  revision: ConversationEnumerationRevision;
}

export interface ConversationCreateRequest {
  route: ProviderInstanceRoute;
  title?: string;
  permissionLevel: string;
  model?: string;
  reasoningEffort?: string;
  workspaceRoot?: string;
  workspaceMode?: string;
  project?: ProviderResourceId;
  extension?: ProviderExtension;
}

export interface ConversationCreateResponse {
  conversation: Conversation;
}

export interface ConversationDeletedEvent {
  conversation: ProviderResourceId;
}

export type ConversationEnumerationRevision = string;

export interface ConversationGetRequest {
  conversation: ProviderResourceId;
  cursor?: Cursor;
  limit?: number;
}

export interface ConversationGetResponse {
  conversation: Conversation;
  items: Array<ConversationItem>;
  pageInfo?: PageInfo;
}

export interface ConversationIdsQuery {
  kind: ConversationIdsQueryKind;
  ids: Array<NativeResourceId>;
}

export type ConversationIdsQueryKind = "ids";

export interface ConversationItemUpsertedEvent {
  item: ConversationItem;
}

export type ConversationListQuery = ConversationUpdatedAfterQuery | ConversationIdsQuery;

export interface ConversationListQueryCapabilities {
  updatedAfter: boolean;
  ids: boolean;
}

export interface ConversationListRequest {
  route: ProviderInstanceRoute;
  projectFilter: ConversationProjectFilter;
  cursor?: Cursor;
  limit?: number;
  query?: ConversationListQuery;
  readerScope?: ReaderScope;
}

export interface ConversationListResponse {
  conversations: Array<Conversation>;
  pageInfo: PageInfo;
}

export interface ConversationMarkReadRequest {
  conversation: ProviderResourceId;
  readerScope: ReaderScope;
  observedActivityVersion: string;
}

export interface ConversationMarkReadResponse {
  readState: ConversationReadState;
}

export type ConversationProjectFilter = ConversationProjectFilterAll | ConversationProjectFilterStandalone | ConversationProjectFilterProject;

export interface ConversationProjectFilterAll {
  kind: ConversationProjectFilterAllKind;
}

export type ConversationProjectFilterAllKind = "all";

export interface ConversationProjectFilterProject {
  kind: ConversationProjectFilterProjectKind;
  project: ProviderResourceId;
}

export type ConversationProjectFilterProjectKind = "project";

export interface ConversationProjectFilterStandalone {
  kind: ConversationProjectFilterStandaloneKind;
}

export type ConversationProjectFilterStandaloneKind = "standalone";

export interface ConversationSearchRequest {
  route: ProviderInstanceRoute;
  searchTerm: string;
  cursor?: Cursor;
  limit?: number;
}

export interface ConversationSearchResponse {
  conversations: Array<Conversation>;
  pageInfo: PageInfo;
}

export interface ConversationUnreadChangedEvent {
  conversation: ProviderResourceId;
  readerScope: ReaderScope;
  readState: ConversationReadState;
  revision: ConversationEnumerationRevision;
}

export interface ConversationUnreadEntry {
  conversation: ProviderResourceId;
  readState: ConversationReadState;
}

export interface ConversationUnreadListRequest {
  route: ProviderInstanceRoute;
  readerScope: ReaderScope;
  cursor?: Cursor;
  limit?: number;
}

export interface ConversationUnreadListResponse {
  conversations: Array<ConversationUnreadEntry>;
  pageInfo: PageInfo;
  revision: ConversationEnumerationRevision;
}

export interface ConversationUpdatedAfterQuery {
  kind: ConversationUpdatedAfterQueryKind;
  updatedAfter: TimestampMs;
}

export type ConversationUpdatedAfterQueryKind = "updatedAfter";

export interface ConversationUpsertedEvent {
  conversation: Conversation;
}

export interface EventSubscribeRequest {
  subscriptionId: string;
}

export interface EventSubscribeResponse {
  subscriptionId: string;
  message: string;
}

export interface EventUnsubscribeRequest {
  subscriptionId: string;
}

export interface EventUnsubscribeResponse {
  subscriptionId: string;
}

export interface HarnessDescriptor {
  id: string;
  displayName: string;
  version?: string;
  executablePath?: string;
}

export interface InstanceCapabilitiesRequest {
  route: ProviderInstanceRoute;
}

export interface InstanceCapabilitiesResponse {
  capabilities: ProviderCapabilities;
}

export interface InstanceCreateRequest {
  route: ProviderInstanceRoute;
  instanceKind: ProviderInstanceKind;
  displayName: string;
  settings: JsonObject;
}

export interface InstanceCreateResponse {
  instance: ProviderInstance;
}

export interface InstanceDestroyRequest {
  route: ProviderInstanceRoute;
}

export interface InstanceDestroyResponse {
  destroyed: boolean;
}

export interface InstanceStartRequest {
  route: ProviderInstanceRoute;
}

export interface InstanceStartResponse {
  instance: ProviderInstance;
}

export type InstanceStatus = "created" | "starting" | "ready" | "stopping" | "stopped" | "error";

export interface InstanceStatusChangedEvent {
  instance: ProviderInstance;
  previousStatus?: InstanceStatus;
}

export interface InstanceStopRequest {
  route: ProviderInstanceRoute;
}

export interface InstanceStopResponse {
  instance: ProviderInstance;
}

export interface ProjectChangedEvent {
  project: ProviderResourceId;
  changeType: ProjectChangeType;
}

export interface ProjectCreateRequest {
  route: ProviderInstanceRoute;
  idempotencyKey: RequestId;
  name: string;
  roots: Array<ProjectRoot>;
  metadata: Record<string, string>;
}

export interface ProjectCreateResponse {
  project: Project;
}

export interface ProjectDeleteRequest {
  project: ProviderResourceId;
}

export interface ProjectDeleteResponse {

}

export interface ProjectGetRequest {
  project: ProviderResourceId;
}

export interface ProjectGetResponse {
  project: Project;
}

export interface ProjectListRequest {
  route: ProviderInstanceRoute;
  cursor?: Cursor;
  limit?: number;
}

export interface ProjectListResponse {
  projects: Array<Project>;
  pageInfo: PageInfo;
}

export interface ProjectUpdateRequest {
  project: ProviderResourceId;
  name?: string;
  roots?: Array<ProjectRoot>;
  metadata?: Record<string, string>;
}

export interface ProjectUpdateResponse {
  project: Project;
}

export interface ProviderCapabilities {
  revision: string;
  methods: Array<ProviderCapability>;
  turnSend?: TurnSendCapabilities;
  conversationCreate?: ConversationCreateCapabilities;
  extensions: Array<ProviderExtension>;
  conversationListQuery?: ConversationListQueryCapabilities;
}

export type ProviderCapability = "project.list" | "project.get" | "project.create" | "project.update" | "project.delete" | "conversation.list" | "conversation.search" | "conversation.get" | "conversation.create" | "turn.start" | "turn.steer" | "turn.interrupt" | "approval.resolve" | "conversation.active.list" | "conversation.unread.list" | "conversation.markRead";

export interface ProviderDescribeRequest {

}

export interface ProviderDescribeResponse {
  plugin: ProviderPluginDescriptor;
}

export interface ProviderExtension {
  namespace: string;
  data: JsonObject;
}

export interface ProviderInitializeRequest {
  hostClientId: ClientId;
  hostDeviceId: DeviceId;
  hostVersion: string;
  supportedVersions: VersionRange;
}

export interface ProviderInitializeResponse {
  selectedVersion: ProtocolVersion;
  plugin: ProviderPluginDescriptor;
}

export interface ProviderInstance {
  route: ProviderInstanceRoute;
  pluginId: ProviderPluginId;
  instanceKind: ProviderInstanceKind;
  displayName: string;
  harness: HarnessDescriptor;
  status: InstanceStatus;
  authentication?: ProviderAuthentication;
  usage?: ProviderUsage;
  capabilities: ProviderCapabilities;
}

export type ProviderInstanceKind = string;

export interface ProviderInstanceRoute {
  deviceId: DeviceId;
  providerPluginId: ProviderPluginId;
  providerInstanceId: ProviderInstanceId;
}

export interface ProviderNotificationEvent {
  subscriptionId: string;
  eventId: string;
  receivedAt: TimestampMs;
  payload: JsonObject;
}

export interface ProviderPingRequest {
  sequence: number;
  hostSessionId: string;
  clients: ClientConnectionsSnapshot;
  instances: Array<ProviderInstanceRoute>;
}

export interface ProviderPingResponse {
  sequence: number;
  clientsRevision: number;
}

export interface ProviderPluginDescriptor {
  pluginId: ProviderPluginId;
  displayName: string;
  version: string;
  defaultWorkspaceRoot?: string;
  supportedVersions: VersionRange;
  instanceKinds: Array<ProviderInstanceKind>;
}

export interface ProviderResourceId {
  deviceId: DeviceId;
  providerPluginId: ProviderPluginId;
  providerInstanceId: ProviderInstanceId;
  nativeResourceId: NativeResourceId;
}

export interface ProviderShutdownRequest {

}

export interface ProviderShutdownResponse {
  accepted: boolean;
}

export interface ProviderTransportHello {
  supportedVersions: Array<number>;
  features: Array<string>;
  receive: ProviderTransportLimits;
}

export interface ProviderTransportLimits {
  maxFramePayloadBytes: number;
  maxEncodedMessageBytes: number;
  maxDecodedMessageBytes: number;
  receiveBudgetBytes: number;
  normalStreams: number;
  smallStreams: number;
  controlStreams: number;
  smallMessageBytes: number;
  smallReceiveBudgetBytes: number;
  controlMessageBytes: number;
  controlReceiveBudgetBytes: number;
  connectionWindowBytes: number;
  idleTimeoutMs: number;
  streamTimeoutMs: number;
}

export interface ProviderTransportSelection {
  selectedVersion: number;
  features: Array<string>;
  receive: ProviderTransportLimits;
}

export type ReaderScope = string;

export interface RuntimeCandidate {
  executablePath: string;
  source: RuntimeCandidateSource;
}

export type RuntimeCandidateSource = "configured" | "environment" | "current-path" | "login-shell" | "macos-application" | "windows-application";

export interface RuntimeGetInstalledRequest {
  refresh?: boolean;
}

export interface RuntimeGetInstalledResponse {
  installed: Array<RuntimeInstallation>;
  selected?: RuntimeInstallation;
  scanning?: boolean;
  scanError?: string;
}

export interface RuntimeInstallation {
  executablePath: string;
  version: string;
  source: RuntimeCandidateSource;
}

export interface RuntimeSelectRequest {
  candidate: RuntimeCandidate;
}

export interface RuntimeSelectResponse {
  selected: RuntimeInstallation;
}

export interface TurnInterruptRequest {
  conversation: ProviderResourceId;
  turn: ProviderResourceId;
}

export interface TurnInterruptResponse {
  turn: TurnTask;
}

export interface TurnOutputDeltaEvent {
  turn: ProviderResourceId;
  conversation: ProviderResourceId;
  itemId: NativeResourceId;
  contentId: NativeResourceId;
  kind: ConversationContentKind;
  delta: string;
  extension?: ProviderExtension;
}

export interface TurnStartRequest {
  conversation: ProviderResourceId;
  clientRequestId: RequestId;
  capabilityRevision: string;
  input: TurnInput;
  selection: TurnSelection;
}

export interface TurnStartResponse {
  accepted: boolean;
  turn: TurnTask;
  userItem: ConversationItem | null;
  effectiveSelection: TurnSelection;
}

export interface TurnSteerRequest {
  conversation: ProviderResourceId;
  turn: ProviderResourceId;
  clientMessageId: RequestId;
  message: string;
}

export interface TurnSteerResponse {
  turn: TurnTask;
}

export interface TurnUpsertedEvent {
  turn: TurnTask;
}

export interface ProtocolRequestMap {
  "conversation.active.list": ConversationActiveListRequest;
  "conversation.unread.list": ConversationUnreadListRequest;
  "conversation.markRead": ConversationMarkReadRequest;
  "provider.ping": ProviderPingRequest;
  "provider.initialize": ProviderInitializeRequest;
  "provider.describe": ProviderDescribeRequest;
  "runtime.getInstalled": RuntimeGetInstalledRequest;
  "runtime.select": RuntimeSelectRequest;
  "instance.create": InstanceCreateRequest;
  "instance.start": InstanceStartRequest;
  "instance.stop": InstanceStopRequest;
  "instance.destroy": InstanceDestroyRequest;
  "instance.capabilities": InstanceCapabilitiesRequest;
  "conversation.list": ConversationListRequest;
  "project.list": ProjectListRequest;
  "project.get": ProjectGetRequest;
  "project.create": ProjectCreateRequest;
  "project.update": ProjectUpdateRequest;
  "project.delete": ProjectDeleteRequest;
  "conversation.search": ConversationSearchRequest;
  "conversation.get": ConversationGetRequest;
  "conversation.acquireInteraction": ConversationAcquireInteractionRequest;
  "conversation.create": ConversationCreateRequest;
  "turn.start": TurnStartRequest;
  "turn.steer": TurnSteerRequest;
  "turn.interrupt": TurnInterruptRequest;
  "approval.resolve": ApprovalResolveRequest;
  "provider.shutdown": ProviderShutdownRequest;
  "event.subscribe": EventSubscribeRequest;
  "event.unsubscribe": EventUnsubscribeRequest;
}

export interface ProtocolResponseMap {
  "conversation.active.list": ConversationActiveListResponse;
  "conversation.unread.list": ConversationUnreadListResponse;
  "conversation.markRead": ConversationMarkReadResponse;
  "provider.ping": ProviderPingResponse;
  "provider.initialize": ProviderInitializeResponse;
  "provider.describe": ProviderDescribeResponse;
  "runtime.getInstalled": RuntimeGetInstalledResponse;
  "runtime.select": RuntimeSelectResponse;
  "instance.create": InstanceCreateResponse;
  "instance.start": InstanceStartResponse;
  "instance.stop": InstanceStopResponse;
  "instance.destroy": InstanceDestroyResponse;
  "instance.capabilities": InstanceCapabilitiesResponse;
  "conversation.list": ConversationListResponse;
  "project.list": ProjectListResponse;
  "project.get": ProjectGetResponse;
  "project.create": ProjectCreateResponse;
  "project.update": ProjectUpdateResponse;
  "project.delete": ProjectDeleteResponse;
  "conversation.search": ConversationSearchResponse;
  "conversation.get": ConversationGetResponse;
  "conversation.acquireInteraction": ConversationAcquireInteractionResponse;
  "conversation.create": ConversationCreateResponse;
  "turn.start": TurnStartResponse;
  "turn.steer": TurnSteerResponse;
  "turn.interrupt": TurnInterruptResponse;
  "approval.resolve": ApprovalResolveResponse;
  "provider.shutdown": ProviderShutdownResponse;
  "event.subscribe": EventSubscribeResponse;
  "event.unsubscribe": EventUnsubscribeResponse;
}

export interface ProtocolEventMap {
  "event.conversationActiveChanged": ConversationActiveChangedEvent;
  "event.conversationUnreadChanged": ConversationUnreadChangedEvent;
  "event.conversationDeleted": ConversationDeletedEvent;
  "event.projectChanged": ProjectChangedEvent;
  "event.instanceStatusChanged": InstanceStatusChangedEvent;
  "event.conversationUpserted": ConversationUpsertedEvent;
  "event.conversationItemUpserted": ConversationItemUpsertedEvent;
  "event.turnUpserted": TurnUpsertedEvent;
  "event.turnOutputDelta": TurnOutputDeltaEvent;
  "event.approvalRequested": ApprovalRequestedEvent;
  "event.approvalResolved": ApprovalResolvedEvent;
  "event.notification": ProviderNotificationEvent;
  "runtime.inventoryChanged": RuntimeGetInstalledResponse;
}

export type ProtocolMethod = keyof ProtocolRequestMap;
export type ProtocolEventName = keyof ProtocolEventMap;
export type ProtocolRequest = {
  [M in ProtocolMethod]: { jsonrpc: "2.0"; id: RequestId; method: M; params: ProtocolRequestMap[M] }
}[ProtocolMethod];
export type ProtocolSuccessResponse<M extends ProtocolMethod = ProtocolMethod> = { jsonrpc: "2.0"; id: RequestId; result: ProtocolResponseMap[M] };
export type ProtocolErrorResponse = { jsonrpc: "2.0"; id: RequestId | null; error: RpcError };
export type ProtocolResponse<M extends ProtocolMethod = ProtocolMethod> = ProtocolSuccessResponse<M> | ProtocolErrorResponse;
export type ProtocolEvent = {
  [E in ProtocolEventName]: { jsonrpc: "2.0"; method: E; params: ProtocolEventMap[E] }
}[ProtocolEventName];

export interface ProtocolTransport {
  request<M extends ProtocolMethod>(method: M, params: ProtocolRequestMap[M]): Promise<ProtocolResponseMap[M]>;
}
