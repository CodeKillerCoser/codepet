...
DNS-SD advertises `_codepet._tcp.local.` and its TXT record is limited to `id`, `name`, `vmin`, `vmax`, and `pair`. `pair` is `1` only while the Host actively offers one-time pairing and is otherwise `0`. Discovery supplies the listener's explicit IP and actual TLS port only; it does not establish trust and must never publish a certificate fingerprint, pairing secret, credential, Provider, project, or conversation data.
