...
<main class="app-shell main-theme light" class:sidebar-collapsed={collapsed}>
  <WindowToolbar bind:collapsed canGoBack={$navigation.canGoBack} canGoForward={$navigation.canGoForward} onBack={navigation.back} onForward={navigation.forward} settingsActive={tab === "settings"} onSettings={() => navigation.navigate("settings")} onError={message => error = message} />
  <aside id="main-sidebar" class="sidebar" inert={collapsed}>
    <MainNavigation current={tab} onNavigate={navigation.navigate} />
  </aside>
  <section class="content-pane"><header class="topbar"><h2>{tab === "settings" ? "设置" : "任务管理"}</h2></header><div class="content">{#if tab === "tasks"}<TaskLineage />{:else if tab === "settings"}<TaskSettingsPage />{:else}<p>其他页面的内容未包含在此测试场景中。</p>{/if}{#if error}<p role="alert">{error}</p>{/if}</div></section>
</main>
