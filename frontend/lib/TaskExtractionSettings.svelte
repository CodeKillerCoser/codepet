  ...
  {#if config}
    <form on:submit|preventDefault={save} on:input={() => saved = false}>
      <div class="fields">
        ...
        <label>运行实例<select bind:value={config.harnessInstanceId}><option value={null}>自动选择唯一实例 / 本机默认</option>{#each options.instances ?? [] as instance}<option value={instance.id}>{instance.name}</option>{/each}</select></label>
        <label>模型<input bind:value={config.model} required maxlength="120" placeholder="haiku" /></label>
        <label>推理强度<select aria-label="推理强度" bind:value={config.reasoningEffort}><option value="low">低 · low</option><option value="medium">中 · medium</option><option value="high">高 · high</option><option value="xhigh">更高 · xhigh</option><option value="max">最高 · max</option></select></label>
      ...
      <p>模型别名和推理档位的实际支持取决于本机 Harness 与模型；默认使用低档模型、低推理强度。配置从下一次运行生效。</p>
      <button type="submit" disabled={saving}>{saving ? "保存中…" : "保存 Agent 设置"}</button>
