...
export interface LineageSnapshot { threads: LineageThread[]; tasks: LineageTask[]; pendingMessages: number; diagnostics: string[]; lastExtraction?: { requestedModel: string; modelUsage: Record<string, unknown> | null; reportedCostUsd: number | null } | null }
...
export interface ExtractionSettings { revision: number; harness: string; harnessInstanceId: string | null; model: string; reasoningEffort: string; automatic: boolean; skill: string; prompt: string; budgetUsd: number; timeoutSeconds: number; intervalSeconds: number; debounceSeconds: number }
...
export interface ExtractionChoice { id: string; displayName: string; enabled?: boolean }
export interface LineageOptions { sources: { id: string; name: string; directory: string | null; agent?: string; error?: string | null; connectionProviderId?: string | null; watch?: LineageWatch; extraction?: ExtractionSettings }[]; claudeExecutable: string | null; defaultModel: string; budgetUsd: number; instances?: { id: string; name: string; harness: string; error?: string | null; controls?: { modelCatalog?: { kind: string; models?: ExtractionChoice[] }; reasoningEffort?: { options: ExtractionChoice[] } } | null }[]; layout?: { root: string; skills: string; extractionWorkspaces: string; taskWorkspaces: string } }
