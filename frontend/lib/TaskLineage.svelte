<script lang="ts">
  ...
  function applyWatch(config?: LineageWatch) { autoScan = config?.enabled ?? false; autoExtract = config?.extract ?? false; remainingJobs = config?.remainingJobs ?? 5; if (config) model = config.model; }
  async function saveWatch() { await operation("保存后台设置", async () => applyWatch(await lineageApi.watch(providerId, { revision: 0, enabled: autoScan || autoExtract, extract: autoExtract, threadId: selectedThreadId || null, model, remainingJobs, lastError: null }))); }
  async function refreshOptions() { await operation("刷新来源", async () => { options = await lineageApi.options(); if (!providerId) providerId = options.sources[0]?.id ?? ""; applyWatch(options.sources.find(s => s.id === providerId)?.watch); }); }
...
<style>
  ...
  @container(max-width:650px){.workspace{display:flex;flex-direction:column}.navigation{max-height:260px;border-bottom:1px solid var(--color-main-divider)}.center{min-height:520px}.inspector{max-height:420px}.controls{gap:6px}.controls input:not([type=checkbox]),.controls select{max-width:135px}.title{padding:12px}}
</style>
