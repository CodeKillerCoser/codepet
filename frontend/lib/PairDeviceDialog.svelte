<script lang="ts">
  ...
  let focusedForCurrentOpen = false;

  $: if (open && !focusedForCurrentOpen) {
    focusedForCurrentOpen = true;
    void focusCloseButton();
  }

  $: if (!open) {
    focusedForCurrentOpen = false;
  }
...
{#if open}
  <div class="modal-scrim">
    <div class="pair-device-dialog pixel-panel" role="dialog" aria-modal="true" aria-labelledby="pair-device-dialog-title" aria-describedby="pair-device-dialog-description" tabindex="-1">
