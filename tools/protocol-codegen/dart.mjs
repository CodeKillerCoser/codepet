...
function emitProtocolMetadata(writer, pkg, ir) {
  ...
  assert(service.kind === "codepet-envelope", `${pkg.id} Dart generator currently supports codepet-envelope services only`);
  ...
  writer.blank();
  emitProtocolCodec(writer, pkg, ir);
}
...
export function generateDart(record, _model, ir) {
  ...
  writer.line("// ignore_for_file: unused_element");
  writer.blank();
  writer.line("import 'dart:convert';");
  if (pkg.dependencies.includes("core-v1")) {
    writer.line("import 'package:codepet_core_sdk/codepet_core_sdk.dart';");
  }
  writer.blank();
